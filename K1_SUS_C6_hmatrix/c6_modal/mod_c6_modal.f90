...
contains
  ...
  subroutine apply_c6_modal_operator_matrix_free(mesh, cfg, active_modes, Jmode, Jmode_next)
    ...
    integer :: d, q_target, q_source, a, source_sector, first_source, last_source, q_cone
    ...
    q_cone=2*cfg%n_face*cfg%n_z_cone
    Jmode_next = (0.0_dp, 0.0_dp)
!$omp parallel do schedule(static) default(shared) private(q_target,q_source,d,a,source_sector,mapA,v,acc,first_source,last_source)
    do q_target = 1, mesh%Q
      allocate(acc(3,size(active_modes)))
      acc = (0.0_dp, 0.0_dp)
      do d = 0, mesh%M - 1
        source_sector = d + 1
        first_source=1; last_source=mesh%Q
        ! Omit the entire coplanar block before invoking pair_map_local.
        if (d==0) then
          if (q_target<=q_cone) then
            first_source=q_cone+1
          else
            last_source=q_cone
          end if
        end if
        do q_source = first_source, last_source
