...
contains
  ...
  subroutine apply_c8_modal_operator_matrix_free(mesh, cfg, active_modes, Jmode, Jmode_next)
...
!$omp end parallel do
    ...
    deallocate(phase)
  end subroutine apply_c8_modal_operator_matrix_free

  subroutine checkpoint_meta_filename(output_dir, filename)
    character(len=*), intent(in) :: output_dir
    character(len=512), intent(out) :: filename
    write(filename,'(A,"/c8_matrix_free_checkpoint_meta.bin")') trim(output_dir)
  ...
  subroutine checkpoint_order_filename(output_dir, order, filename)
    ...
    character(len=512), intent(out) :: filename
    write(filename,'(A,"/c8_matrix_free_order_",I6.6,".bin")') trim(output_dir), order
  ...
  subroutine load_checkpoint_meta(output_dir, q_expected, sector_expected, max_order_expected, &
    ...
    if (magic /= 'C8MF002 ') stop 'Invalid C8 matrix-free checkpoint meta magic.'
    if (q_file /= q_expected) stop 'C8 matrix-free checkpoint panel count mismatch.'
    if (sector_file /= sector_expected) stop 'C8 matrix-free checkpoint sector count mismatch.'
    if (max_order_file /= max_order_expected) stop 'C8 matrix-free checkpoint max_order mismatch.'
    if (last_order < 0 .or. last_order > max_order_expected) stop 'Invalid C8 matrix-free checkpoint last_order.'
    if (nactive_file /= size(active_modes_expected)) stop 'C8 matrix-free checkpoint active-mode count mismatch.'
    if (any(active_modes_file /= active_modes_expected)) stop 'C8 matrix-free checkpoint active-mode list mismatch.'
  ...
  subroutine load_checkpoint(output_dir, order, active_modes, Jmode)
    ...
    allocate(active_modes_file(nactive_file))
    read(u) active_modes_file
    if (magic /= 'C8MFO02 ') stop 'Invalid C8 matrix-free checkpoint order magic.'
    if (order_file /= order) stop 'C8 matrix-free checkpoint order mismatch.'
    if (q_file /= size(Jmode,2)) stop 'C8 matrix-free checkpoint order panel count mismatch.'
    if (nactive_file /= size(active_modes)) stop 'C8 matrix-free checkpoint order active-mode count mismatch.'
    if (any(active_modes_file /= active_modes)) stop 'C8 matrix-free checkpoint order active-mode list mismatch.'
    read(u) Jmode
    close(u)
    deallocate(active_modes_file)
  end subroutine load_checkpoint

  subroutine run_c8_modal_ipo(mesh, cfg, J0, max_order, stop_ratio, orders, ratios, step_times, &
    ...
    if (loaded_last < max_order) then
      do order = loaded_last + 1, max_order
        call cpu_time(t0)
        call apply_c8_modal_operator_matrix_free(mesh, cfg, active_modes, Jmode, Jmode_next)
        call cpu_time(t1)
  ...
  end subroutine run_c8_modal_ipo

end module mod_c8_modal
