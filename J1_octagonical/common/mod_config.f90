module mod_config
  ...
  type :: sim_config_type
    ...
    integer :: run_mode                  ! 1=full only, 2=C8 modal only, 3=postprocess compare
    real(dp) :: stop_ratio
    ...
    integer :: mode_policy                 ! 0=all, 1=m=1,7, 2=automatic
...
contains
  ...
  subroutine set_defaults(cfg)
    ...
    cfg%use_plane_wave=.false.; cfg%M=8
    ...
    cfg%save_order_exit_fields=.true.; cfg%max_dense_gib=2.0_dp
  ...
  subroutine load_config(filename,cfg)
    ...
    logical :: save_order_exit_fields
    character(len=256) :: output_dir
    namelist /simulation/ freq,e0,w0_src,z0_src,beam_offset_x,beam_offset_y,incidence_theta_deg, &
      ...
      compute_fields,save_order_exit_fields,max_dense_gib,estimated_pair_rate,mode_policy, &
    ...
    call update_derived_config(cfg)
    if (cfg%M/=8) then
      write(*,'(A)') 'A regular octagon requires M=8, not M=16.'
      stop 1
    end if
  ...
  subroutine estimate_problem(cfg,Q,N,dense_gib,c8_gib,pairs_full,pairs_c8,seconds_full)
    type(sim_config_type), intent(in) :: cfg
    integer, intent(out) :: Q,N
    real(dp), intent(out) :: dense_gib,c8_gib,seconds_full
    integer(int64), intent(out) :: pairs_full,pairs_c8
    Q=2*cfg%n_face*(cfg%n_z_cone+cfg%n_z_pipe); N=cfg%M*Q
    pairs_full=int(N,int64)*int(N,int64); pairs_c8=int(cfg%M,int64)*int(Q,int64)*int(Q,int64)
    dense_gib=16.0_dp*real((3_int64*N)*(3_int64*N),dp)/1024.0_dp**3
    c8_gib=16.0_dp*real((3_int64*Q)*(3_int64*Q)*cfg%M,dp)/1024.0_dp**3
  ...
  subroutine write_preflight(cfg,filename)
    ...
    write(u,'(A,I0)') 'c8_pair_interactions_per_order=',pc
    write(u,'(A,ES24.16)') 'full_dense_gib=',gf; write(u,'(A,ES24.16)') 'c8_blocks_gib=',gc
