#!/usr/bin/env python3
"""
Visualize the G2 soft-hexagon-to-circle cone geometry used for the closed-cap PO model.

This script does not run the electromagnetic solver. It only reproduces the wall-shape
formula so that the rounded hexagon and its G2 connection to the circular pipe can be
checked quickly.
...
Interpretation:
  01_cross_sections_xy.svg checks whether the inlet is a rounded hexagon and gradually becomes circular.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Visualize the G2 soft-hexagon-to-circle concentrator geometry.")
    ...
    parser.add_argument("--beta", type=float, default=80.0, help="Softmax beta. Larger means closer to a sharp hexagon. Default: 80")
