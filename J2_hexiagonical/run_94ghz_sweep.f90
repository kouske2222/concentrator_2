program run_94ghz_sweep
  ...
  real(dp) :: full_gib,c6_gib,seconds,input_freq
  integer(int64) :: full_pairs,c6_pairs
  ...
  write(u,'(A)') 'frequency_hz,polarization_deg,offset_mm,incidence_deg,panels_per_sector,total_panels,'// &
                 'full_pairs_per_order,c6_pairs_per_order,full_dense_gib,c6_blocks_gib,'// &
  ...
  do i=1,size(frequencies)
    ...
    call estimate_problem(cfg,Q,N,full_gib,c6_gib,full_pairs,c6_pairs,seconds)
    do j=1,size(polarizations); do k=1,size(offsets_mm); do l=1,size(angles_deg)
      write(u,'(ES16.8,3(",",F8.3),2(",",I0),2(",",I0),2(",",ES16.8),",",L1,",",L1)') &
        cfg%freq,polarizations(j),offsets_mm(k),angles_deg(l),Q,N,full_pairs,c6_pairs, &
        full_gib,c6_gib,.true.,full_gib<=cfg%max_dense_gib
  ...
  write(*,'(A,ES12.4)') 'C6 panel pairs per reflection/order = ',real(c6_pairs,dp)
  if (c6_gib>cfg%max_dense_gib) then
    write(*,'(A)') 'Dense C6 storage is unsafe. Use matrix-free GPU/FMM or an admissible-block H-matrix/ACA.'
