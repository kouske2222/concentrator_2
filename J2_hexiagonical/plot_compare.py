#!/usr/bin/env python3
"""Create common-scale full/C6 comparison figures from Fortran CSV output."""
...
def create_figures(result: Path) -> None:
    ...
    for column, label in enumerate(labels):
        for row, (data, values, method) in enumerate(((full, vf[column], "full"), (modal, vm[column], "C6 modal"))):
    ...
    fig.savefig(out / "xz_full_vs_c6.png", dpi=180, bbox_inches="tight")
    plt.close(fig)
    ...
    if order_path.exists():
        ...
        axes[0].semilogy(order["order"], order["fast_ratio"], "+:", label="m=1,5")
    ...
    ax.set(xlabel="C6 mode m", ylabel="DFT current norm")
    ...
    axes[0].plot(am["z_m"]*1e3, am["E_mag"], "--", label="C6")
    axes[0].set_ylabel("|E| [V/m]"); axes[0].legend()
    axes[1].plot(af["z_m"]*1e3, np.unwrap(np.angle(exf)), label="full")
    axes[1].plot(am["z_m"]*1e3, np.unwrap(np.angle(exm)), "--", label="C6")
    ...
    for index, (key, title) in enumerate((("Jfull_mag", "full"), ("Jmodal_mag", "C6 modal")), 1):
    ...
    for plane in range(1, 4):
        ...
        for ax, data, title in ((axes[0], pf, "full"), (axes[1], pm, "C6 modal")):
        ...
        fig.savefig(out / f"xy{plane}_full_vs_c6.png", dpi=180)
        plt.close(fig)
        ...
        axes[0].plot(xcut[order], pm["E_mag"][center][order], "--", label="C6 modal")
    ...
    labels = ["full operator", "C6 blocks"]
    memory = [float(info["full_operator_mib"]), float(info["c6_blocks_mib"])]
    build = [float(info["full_operator_build_s"]), float(info["c6_blocks_build_s"])]
