program compare_full_vs_c6
  ...
  real(dp) :: initial_modes(6),modal_initial_modes(6),modal_final_modes(6),fast_final_modes(6)
  ...
  real(dp) :: dense_gib,c6_gib,estimated_seconds
  real(dp) :: full_total_s,modal_total_s,full_step_sum_s,modal_step_sum_s
  integer(int64) :: pairs_full,pairs_c6,full_elements
  ...
  call estimate_problem(cfg,Q_est,N_est,dense_gib,c6_gib,pairs_full,pairs_c6,estimated_seconds)
  write(*,'(A,F12.3)') 'Estimated full dense storage [GiB] = ',dense_gib
  write(*,'(A)') 'Estimated C6 block storage [GiB] =        0.000 (matrix-free)'
  ...
  case (2)
    write(*,'(A)') 'Run mode 2: C6 modal matrix-free IPO only.'
    allocate(J0(3,mesh%Q,mesh%M))
    call build_initial_po_current(mesh,cfg,J0,initial_modes)
    call run_c6_modal_ipo(mesh,cfg,J0,cfg%max_reflection_order,cfg%stop_ratio, &
    ...
    if (cfg%compute_fields .and. cfg%write_mode_fields) call write_c6_mode_fields(mesh,cfg,Jmodal)
    call cpu_time(total_t1)
  ...
  case (3)
    ...
    if (cfg%compute_fields) then
      ...
      if (cfg%write_mode_fields) call write_c6_mode_fields(mesh,cfg,Jmodal)
    end if
  ...
  case default
    write(*,'(A,I0)') 'Invalid run_mode. Use 1=full, 2=C6 modal, or 3=postprocess. run_mode = ',cfg%run_mode
    stop 1
  end select

  write(*,'(A)') 'C6 PO/IPO comparison completed.'
...
contains
  ...
  subroutine compare_and_write_fields(msh,conf,Jf,Jm,err_xz,err_axis,err_exit,pf,pm)
    ...
    do iz=1,conf%nz_xz; do ix=1,conf%nx_xz
      ...
      mask(i)=inside_hexagon(x(i),y(i),z(i),conf,conf%wall_clearance_fraction)
    end do; end do
  ...
  subroutine write_c6_mode_fields(msh,conf,Jcurrent)
    type(panel_mesh_type), intent(in) :: msh
    type(sim_config_type), intent(in) :: conf
    complex(dp), intent(in) :: Jcurrent(3,msh%Q,msh%M)
    complex(dp), allocatable :: Jhat(:,:,:),Jone(:,:,:),E(:,:),H(:,:)
    real(dp), allocatable :: x(:),y(:),z(:),mode_norms(:)
    logical, allocatable :: mask(:)
    real(dp) :: zexit,spacing,max_e,phase_step_deg
    character(len=512) :: filename
    integer :: m,signed_m,u

    allocate(Jhat(3,msh%Q,msh%M),Jone(3,msh%Q,msh%M),mode_norms(msh%M))
    call decompose_current_all_modes(Jcurrent,Jhat,mode_norms)
    zexit=conf%base_z+conf%l_cone+conf%l_pipe-0.25_dp*min(conf%l_cone/real(conf%n_z_cone,dp), &
                                                        conf%l_pipe/real(conf%n_z_pipe,dp))
    call make_xy_grid(conf,zexit,x,y,z,mask,spacing)
    allocate(E(3,size(x)),H(3,size(x)))
    open(newunit=u,file=trim(conf%output_dir)//'/mode_field_summary.csv',status='replace',action='write')
    write(u,'(A)') 'mode,signed_azimuthal_order,phase_step_deg,cumulative_dft_current_norm,max_induced_E_V_per_m'
    do m=0,msh%M-1
      call reconstruct_active_modes([m],Jhat(:,:,m+1:m+1),Jone)
      call field_from_current(msh,conf,Jone,x,y,z,E,H,.false.)
      call zero_masked(E,H,mask)
      max_e=maxval(sqrt(sum(abs(E)**2,dim=1)),mask=mask)
      signed_m=m
      if (m>msh%M/2) signed_m=m-msh%M
      phase_step_deg=360.0_dp*real(signed_m,dp)/real(msh%M,dp)
      write(filename,'(A,"/mode_m",I0,"_exit.csv")') trim(conf%output_dir),m
      call write_field_csv(trim(filename),x,y,z,E,mask)
      write(u,'(I0,",",I0,3(",",ES24.16))') m,signed_m,phase_step_deg,mode_norms(m+1),max_e
    end do
    close(u)
    deallocate(Jhat,Jone,E,H,x,y,z,mode_norms,mask)
  end subroutine write_c6_mode_fields
  ...
  subroutine write_metrics(conf,msh,ecirc,ej,efast,exz,eaxis,eexit,pf,pm,tbuild,tblock,ttotal, &
    ...
    write(u,'(A,ES24.16)') 'full_operator_build_s=',tbuild; write(u,'(A,ES24.16)') 'c6_blocks_build_s=',tblock
    ...
    write(u,'(A,I0)') 'c6_stored_complex_elements=',c6_count
    write(u,'(A,ES24.16)') 'full_operator_mib=',full_mib; write(u,'(A,ES24.16)') 'c6_blocks_mib=',block_mib
    ...
    write(u,'(A,F8.3)') 'ideal_speedup_modes_1_5=',real(conf%M*conf%M,dp)/2.0_dp
    close(u)
  end subroutine write_metrics

end program compare_full_vs_c6
