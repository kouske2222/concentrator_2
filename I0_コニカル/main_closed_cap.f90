program main_closed_cap
  ...
  print *, '========================================'
  print *, 'Microwave PO / IPO: closed parabolic cap'
  print *, 'PTD: OFF'
  ...
  ! Step 1: geometry
  ! ============================================================
  print *, 'Step 1: Generate cone + pipe + parabolic-cap panels'
  ...
  call export_geometry_summary( &
      trim(cfg%OUTPUT_DIR)//'/geometry_summary.txt', &
      cfg%r_cone_in, cfg%r_pipe, cfg%l_cone, cfg%l_pipe, &
  ...
  call build_iterative_po_currents_fast( &
      ...
      cfg%max_reflection_order, cfg%k0, cfg%eta0, &
      cfg%w0_src, cfg%z0_src, cfg%zR_src, &
      cfg%exclusion_factor_wall, cfg%stop_ratio, &
      currents_x, currents_y, currents_z, ratios, order_build_times, n_orders_done, &
      show_progress=cfg%SHOW_IPO_PROGRESS, &
      n_progress_updates=cfg%IPO_PROGRESS_UPDATES, &
      checkpoint_dir=trim(cfg%IPO_CHECKPOINT_DIR), &
      restart_from_checkpoint=cfg%RESTART_FROM_CHECKPOINT)

  print *, '  done'
  print *, '  IPO orders used = ', n_orders_done

  call sum_currents_by_order( &
      currents_x, currents_y, currents_z, &
      n_orders_done, Jx_tot, Jy_tot, Jz_tot)

  call export_ipo_summary( &
      trim(cfg%OUTPUT_DIR)//'/ipo_order_summary.txt', &
      ratios, order_build_times, n_orders_done)
  ...
  if (cfg%RUN_FIELD_MAPS) then
    ...
    if (cfg%RUN_XZ) then
      call export_xz_field_map( &
          cfg, Xm, Ym, Zm, dSm, panel_len_m, &
          Jx_tot, Jy_tot, Jz_tot)
    end if

    if (cfg%RUN_XY) then
      call export_xy_field_maps( &
          cfg, Xm, Ym, Zm, dSm, panel_len_m, &
          Jx_tot, Jy_tot, Jz_tot, xy_plane_z_list)
    end if
  ...
  deallocate(Xm, Ym, Zm, nxm, nym, nzm, dSm, panel_len_m, zone_ids)
  deallocate(Jx_tot, Jy_tot, Jz_tot)
  deallocate(currents_x, currents_y, currents_z)
  deallocate(ratios, order_build_times)
...
contains
  ...
  subroutine export_xz_field_map( &
      cfg_in, Xm_in, Ym_in, Zm_in, dSm_in, panel_len_in, &
      Jx_in, Jy_in, Jz_in)

    type(sim_config_type), intent(in) :: cfg_in
    real(dp), intent(in) :: Xm_in(:), Ym_in(:), Zm_in(:)
    real(dp), intent(in) :: dSm_in(:), panel_len_in(:)
    ...
    integer :: Nx_l, Nz_l, ntot_l
    integer :: ix_l, iz_l, idx_l
    integer :: ibeg_l, iend_l
    real(dp) :: x_min_l, x_max_l, z_min_l, z_max_l
    real(dp) :: dx_l, dz_l
    ...
    if (Nx_l < 2) stop 'NX_XZ must be >= 2.'
    if (Nz_l < 2) stop 'NZ_XZ must be >= 2.'
    if (cfg_in%BLOCK_SIZE_XZ <= 0) stop 'BLOCK_SIZE_XZ must be positive.'
    ...
    x_half_l = (1.0_dp + cfg_in%field_outer_margin_factor_xz) &
        * max(cfg_in%r_cone_in, cfg_in%r_pipe)
    ...
    do iz_l = 1, Nz_l
      do ix_l = 1, Nx_l
        ...
        ! Use direction-dependent wall radius.
        ! This is safer for polygonal sections such as hexagon/heptagon,
        ! provided wall_radius_at_xy_z is implemented for that geometry.
        r_wall_l = wall_radius_at_xy_z( &
            xg(idx_l), yg(idx_l), zg(idx_l), &
    ...
    print *, '  XZ wall_clearance [m] = ', wall_clearance_l
    print *, '  XZ active points      = ', count(mask), '/', ntot_l
...
!$omp end parallel do
    ...
    call export_field_map_binary( &
        trim(cfg_in%OUTPUT_DIR)//'/field_map_xz.bin', &
        ntot_l, xg, yg, zg, esq)

    print *, '  XZ field map exported: ', &
        trim(cfg_in%OUTPUT_DIR)//'/field_map_xz.bin'

    deallocate(xg, yg, zg, esq, mask)
    deallocate(Ex_tot, Ey_tot, Ez_tot)
    deallocate(Ex_sca, Ey_sca, Ez_sca)
  ...
  subroutine export_xy_field_maps( &
    ...
    integer :: ix_l, iy_l, izp_l, idx_l
    integer :: ibeg_l, iend_l
    real(dp) :: z_l, r_wall_l, r_plot_l
    real(dp) :: wall_clearance_l
    real(dp) :: r_cell_l
    real(dp) :: x_min_l, x_max_l, y_min_l, y_max_l
    real(dp) :: dx_l, dy_l
    ...
    if (Nx_l < 2) stop 'NX_XY must be >= 2.'
    if (Ny_l < 2) stop 'NY_XY must be >= 2.'
    if (cfg_in%BLOCK_SIZE_XY <= 0) stop 'BLOCK_SIZE_XY must be positive.'
    ...
    print *, '  XY wall_clearance [m] = ', wall_clearance_l
    ...
    do izp_l = 1, size(z_list)
      ...
      do iy_l = 1, Ny_l
        do ix_l = 1, Nx_l
          ...
          ! Direction-dependent wall radius.
          ! For circular geometry this is equivalent to wall_radius_at_z.
          ! For polygonal geometry this must return the boundary radius
      ...
      write(*,'(A,I0,A,ES13.5)') &
          '  XY plane ', izp_l, ' z [m] = ', z_l
      write(*,'(A,I0,A,I0)') &
          '    active points = ', count(mask), '/', ntot_l
...
!$omp end parallel do
      ...
      write(fname_l,'(A,"/field_map_xy_z",I3.3,".bin")') &
          trim(cfg_in%OUTPUT_DIR), izp_l

      call export_field_map_binary(trim(fname_l), ntot_l, xg, yg, zg, esq)

      write(*,'(A,I0,A,A)') &
          '  XY plane ', izp_l, ' exported: ', trim(fname_l)
    end do
