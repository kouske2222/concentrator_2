...
Purpose:
  ...
  - For XY maps, draw the expected soft-octagon / circle wall boundary at each z.
...
def softmax_cos(theta: np.ndarray | float, beta: float) -> np.ndarray | float:
    theta_arr = np.asarray(theta, dtype=float)
    alphas = math.pi/8.0 + np.arange(8, dtype=float)*math.pi/4.0
    a = beta * np.cos(theta_arr[..., None] - alphas[None, :])
    amax = np.max(a, axis=-1)
    return (amax + np.log(np.sum(np.exp(a - amax[..., None]), axis=-1))) / beta


def soft_octagon_radius(theta: np.ndarray | float, r_vertex: float, beta: float) -> np.ndarray | float:
    m_vertex = softmax_cos(0.0, beta)
    m_theta = softmax_cos(theta, beta)
...
def wall_radius_theta_z(theta: np.ndarray | float, z: np.ndarray | float, cfg: Config) -> np.ndarray | float:
    ...
    if np.any(cone):
        ...
        s = smoothstep_g2(t)
        r8 = soft_octagon_radius(theta_b[cone], cfg.r_cone_in, cfg.beta)
        rw[cone] = (1.0 - s)*r8 + s*cfg.r_pipe
    if np.any(pipe):
        rw[pipe] = cfg.r_pipe
    if np.any(cap):
...
def plot_shape_only(cfg: Config, out_dir: Path, file_format: str = "png", dpi: int = 200) -> None:
    ...
    ax.set_title("Entrance soft-octagon radius vs angle")
    ax.grid(True)
    fig.tight_layout()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize field_map*.bin with soft-octagon wall overlay.")
...
def main() -> None:
    ...
    for m in maps:
        if m.kind == "XY":
            ...
            rs = wall_radius_theta_z(math.pi/8.0, m.const_value, cfg)
            contrast = 100.0*(rv-rs)/rv if np.isfinite(rv) and rv != 0.0 else np.nan
