#!/usr/bin/env python3
"""
Visualize the G2 soft-heptagon-to-circle cone geometry used for the closed-cap PO model.

This script does not run the electromagnetic solver. It only reproduces the wall-shape
formula so that the rounded heptagon and its G2 connection to the circular pipe can be
checked quickly.
...
def smoothmax_polygon_cos(theta: np.ndarray, beta: float) -> np.ndarray:
    """Smooth approximation to max_k cos(theta - alpha_k)."""
    theta = np.asarray(theta)
    alphas = math.pi/POLYGON_N_SIDES + np.arange(POLYGON_N_SIDES) * 2.0*math.pi/POLYGON_N_SIDES
...
def plot_cross_sections(out_dir: Path, cfg: GeometryConfig, beta: float, ntheta: int) -> None:
    ...
    plt.xlabel("x [mm]")
    plt.ylabel("y [mm]")
    plt.title("Cross-sections of soft heptagon to circle transition")
...
def plot_xz_cuts(out_dir: Path, cfg: GeometryConfig, beta: float, nz: int) -> None:
    ...
    theta_list = [0.0, math.pi/(2.0*POLYGON_N_SIDES), math.pi/POLYGON_N_SIDES]
    labels = ["vertex direction theta=0", "between", f"side-center theta=pi/{POLYGON_N_SIDES}"]
...
def plot_g2_derivatives(out_dir: Path, cfg: GeometryConfig, beta: float, nz: int) -> Tuple[float, float]:
    ...
    theta_list = [0.0, math.pi/POLYGON_N_SIDES]
    labels = ["vertex direction theta=0", f"side-center theta=pi/{POLYGON_N_SIDES}"]
...
def plot_3d_surface(out_dir: Path, cfg: GeometryConfig, beta: float, ntheta: int, nz: int) -> None:
    ...
    ax.set_title("3D wireframe: soft heptagon cone connected to circular pipe")
...
def write_summary(out_dir: Path, cfg: GeometryConfig, beta: float, max_d1: float, max_d2: float) -> None:
    ...
    r0 = smooth_polygon_radius(theta, cfg.r_cone_in, beta)
    sharp_inradius = cfg.r_cone_in * math.cos(math.pi/POLYGON_N_SIDES)

    text = f"""Geometry visualization summary
================================

Formula:
  r(theta,t) = (1 - s(t)) rN_soft(theta) + s(t) r_pipe
  s(t)      = 6 t^5 - 15 t^4 + 10 t^3

Soft heptagon:
  m_beta(theta) = (1/beta) log sum_k exp(beta cos(theta - alpha_k))
  alpha_k       = pi/N + 2 k pi/N,  k=0..N-1
  rN_soft(theta)= r_cone_in * m_beta(0) / m_beta(theta)

Parameters:
  N_sides    = {POLYGON_N_SIDES:d}
  beta       = {beta:.6g}
...
Heptagon check at t=0:
  max radius                         = {np.nanmax(r0):.9e} m
  min radius                         = {np.nanmin(r0):.9e} m
  min/max ratio of soft heptagon       = {np.nanmin(r0)/np.nanmax(r0):.9f}
  sharp regular-{POLYGON_NAME} inradius/R    = {math.cos(math.pi/POLYGON_N_SIDES):.9f}
  sharp regular-heptagon inradius      = {sharp_inradius:.9e} m
...
Interpretation:
  01_cross_sections_xy.svg checks whether the inlet is a rounded heptagon and gradually becomes circular.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Visualize the G2 soft-heptagon-to-circle concentrator geometry.")
    ...
    parser.add_argument("--beta", type=float, default=80.0, help="Softmax beta. Larger means closer to a sharp heptagon. Default: 80")
