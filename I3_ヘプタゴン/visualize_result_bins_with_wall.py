...
Purpose:
  ...
  - For XY maps, draw the expected soft-heptagon / circle wall boundary at each z.
...
def plot_shape_only(cfg: Config, out_dir: Path, file_format: str = "svg", dpi: int = 200) -> None:
    ...
    ax.set_title("Entrance soft-heptagon radius vs angle")
    ax.grid(True)
    fig.tight_layout()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Visualize field_map*.bin with soft-heptagon wall overlay.")
