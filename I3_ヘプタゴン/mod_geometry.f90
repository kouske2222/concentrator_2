module mod_geometry
  ...
  public :: polygon_softmax_beta_default
  public :: polygon_n_sides_default
...
contains
  ...
  pure real(dp) function wall_radius_at_z( &
    ...
    if (z < base_z .or. z > z_cap_vertex) then
      ...
      t = zloc / l_cone
      s = delayed_polygon_to_circle_weight(t)

      ! Plot / grid-size radius for tapered heptagon-to-circle geometry.
      rw = (1.0_dp - s) * polygonal_taper_radius(t, r_cone_in, r_pipe) + s * r_pipe
  ...
  pure integer function polygon_n_sides_default() result(n_sides)
    ! Number of sides for the polygonal cone section.
    n_sides = 7
  end function polygon_n_sides_default

  pure real(dp) function polygon_softmax_beta_default() result(beta)
    ! Softmax sharpness for the polygonal wall.
    ! 80.0_dp matches the geometry-check / wall-overlay scripts and gives a clearly polygonal shape.
    ! Set this to 1.0_dp only if you intentionally want the almost-circular softness of the uploaded base file.
    beta = 80.0_dp
  end function polygon_softmax_beta_default
  ...
  pure real(dp) function smootherstep_local(t) result(s)
    ...
    pure real(dp) function polygonal_taper_radius(t, r_cone_in, r_pipe) result(rref)
    real(dp), intent(in) :: t
    ...
    ! Linear taper in z.
    ! t = 0 : entrance heptagon radius
    ! t = 1 : pipe radius
    rref = r_cone_in + (r_pipe - r_cone_in) * tt

  end function polygonal_taper_radius

  ! Delayed transition from soft heptagon prism to circular pipe.
  ! t = 0.0 ... 1.0 corresponds to the cone section.
  ! For t <= polygon_hold_fraction_default(), the cross-section remains a
  ! straight soft-heptagon prism.  Between t0 and t1 it changes smoothly
  ! to a circle.  At both ends, ds/dt = d2s/dt2 = 0.
  pure real(dp) function polygon_hold_fraction_default() result(t0)

    t0 = 0.9_dp

  end function polygon_hold_fraction_default


  pure real(dp) function polygon_transition_end_fraction_default() result(t1)

    t1 = 1.00_dp

  end function polygon_transition_end_fraction_default


  pure real(dp) function delayed_polygon_to_circle_weight(t) result(s)
    real(dp), intent(in) :: t

    real(dp) :: t0, t1, u

    t0 = polygon_hold_fraction_default()
    t1 = polygon_transition_end_fraction_default()
  ...
  end function delayed_polygon_to_circle_weight


  pure real(dp) function soft_polygon_radius(theta, r_vertex) result(rpoly)
    ...
    pi_l = acos(-1.0_dp)
    beta = polygon_softmax_beta_default()
    n_sides = polygon_n_sides_default()
    ...
    max_arg = -huge(1.0_dp)
    do k = 0, n_sides - 1
      alpha = pi_l / real(n_sides, dp) + real(k, dp) * 2.0_dp*pi_l / real(n_sides, dp)
    ...
    rpoly = r_vertex * m_vertex / m_theta

  end function soft_polygon_radius
  ...
  pure real(dp) function blended_cone_radius(theta, t, r_cone_in, r_pipe) result(rw)
      ...
      ! Heptagon-to-circle transition weight.
      ! s = 0 : heptagon cross-section
      ! s = 1 : circular cross-section
      s = delayed_polygon_to_circle_weight(t)

      ! z-dependent reference radius.
      ! This makes the heptagon part a tapered frustum, not a prism.
      rref = polygonal_taper_radius(t, r_cone_in, r_pipe)

      ! Soft heptagon whose size decreases along z.
      rpoly = soft_polygon_radius(theta, rref)

      ! Blend from tapered heptagon to circular pipe.
      rw = (1.0_dp - s) * rpoly + s * r_pipe
  ...
  subroutine define_closed_concentrator_panels_adaptive( &
    ...
    do i = 1, N_z_cone
      ...
      t_c = zc / l_cone
      s_c = delayed_polygon_to_circle_weight(t_c)
      ! Maximum radius at this z.  The actual radius still depends on theta.
      r_count = (1.0_dp - s_c) * polygonal_taper_radius(t_c, r_cone_in, r_pipe)  + s_c * r_pipe
    ...
    ! ============================================================
    ! zone 0: soft-heptagon prism followed by G2 transition to circular pipe
    !
    ! Surface parameterization:
    !   S(theta,z) = [r(theta,t) cos(theta),
    !                 r(theta,t) sin(theta),
    !                 base_z + z]
    !   t = z / l_cone; transition starts after polygon_hold_fraction_default()
