...
contains
  ...
  subroutine set_defaults(cfg)
    ...
    cfg%freq=170.0e9_dp; cfg%e0=1.0_dp; cfg%w0_src=0.0204_dp; cfg%z0_src=-2.2_dp
  ...
  subroutine load_config(filename,cfg)
    ...
    call update_derived_config(cfg)
    if (abs(cfg%freq-170.0e9_dp)>1.0_dp) error stop 'K1 supports only 170 GHz.'
    if (min(cfg%n_face,cfg%n_z_cone,cfg%n_z_pipe)<1) error stop 'Invalid mesh counts.'
    if (min(cfg%d_in,cfg%d_out,cfg%l_cone,cfg%l_pipe,cfg%w0_src)<=0) error stop 'Invalid lengths.'
    if (cfg%visibility_samples<1) error stop 'visibility_samples must be positive.'
    if (cfg%M/=6) then
