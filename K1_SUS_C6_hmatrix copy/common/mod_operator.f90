module mod_operator
  ...
  implicit none
  private
  public :: pair_map_local, build_full_operator, visible_pair
...
contains
  ...
  subroutine pair_map_local(mesh,cfg,pt,qt,ps,qs,A)
    ...
    integer :: column
    A=(0.0_dp,0.0_dp)
    ! Same sector and same planar piece is identically grazing.
    if (pt==ps) then
      if ((qt<=2*cfg%n_face*cfg%n_z_cone) .eqv. (qs<=2*cfg%n_face*cfg%n_z_cone)) return
    end if
    ...
    ! Conservative rejection before sqrt. Keep the original normalized
    ! tolerance for all surviving pairs; no new distance/angular cutoff.
    if (sum(rvec*ns)<0.0_dp .or. sum(rvec*nt)>0.0_dp) return
