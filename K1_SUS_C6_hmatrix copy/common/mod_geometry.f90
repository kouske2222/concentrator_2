module mod_geometry
  ...
  implicit none
  private
  public :: panel_mesh_type, build_c6_mesh, radius_at_z, inside_hexagon
...
contains
  ...
  pure real(dp) function circumradius(diameter, across_vertices) result(radius)
    ...
    if (across_vertices) then
      ...
      radius = 0.5_dp * diameter / cos(PI/6.0_dp)
    end if
  ...
  subroutine build_c6_mesh(cfg, mesh)
    type(sim_config_type), intent(in) :: cfg
    ...
    do iz=1,nztot+1
      ...
      do iu=0,cfg%n_face
        ...
        grid(iz,iu+1,1)=r*((1.0_dp-u)*cos(-PI/6.0_dp)+u*cos(PI/6.0_dp))
        grid(iz,iu+1,2)=r*((1.0_dp-u)*sin(-PI/6.0_dp)+u*sin(PI/6.0_dp))
  ...
  end subroutine build_c6_mesh

  pure logical function inside_hexagon(x,y,z,cfg,clearance_fraction) result(ok)
    ...
    apothem=radius_at_z(z,cfg)*cos(PI/6.0_dp)*(1.0_dp-clearance_fraction)
    do p=0,cfg%M-1
